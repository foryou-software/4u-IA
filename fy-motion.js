...
(() => {
  ...
  /* fyLoop(tick, opts) → { stop() }
     ...
     opts.reduced   "off" (default: bajo reduced-motion no corre)
                    "once" (un render estático: opts.onceTick o tick, una vez)
                    "run"  (es dato, no decoración: sigue corriendo)
     opts.onceTick  render estático alternativo para reduced "once"
     opts.onstate   hook (running) — p. ej. video.play()/pause(); tick puede ser null */
